...
options(scipen = 999)
set.seed(204)
setwd("/Users/evahenningsen/Documents/MPGI_paper/Figure_6")

## Brachy plot loop

Bd21_files <- list.files(path = "/Users/evahenningsen/Documents/MPGI_paper/Figure_6/Bd21", pattern = ".txt", full.names = TRUE, recursive = FALSE)
...
W2691_files <- list.files(path = "/Users/evahenningsen/Documents/MPGI_paper/Figure_6/W2691", pattern = ".txt", full.names = TRUE, recursive = FALSE)
...
Sr9b_files <- list.files(path = "/Users/evahenningsen/Documents/MPGI_paper/Figure_6/Sr9b", pattern = ".txt", full.names = TRUE, recursive = FALSE)
...
bd_max_subsets <- lapply(bd_cluster_list, FUN = function(x){
  subset(Bd21_max, cluster == x)
})

W2691_max_subsets <- lapply(W2691_cluster_list, FUN = function(x){
  subset(W2691_max, cluster == x)
})

Sr9b_max_subsets <- lapply(Sr9b_cluster_list, FUN = function(x){
  subset(Sr9b_max, cluster == x)
})
###
...
Bd21_plotting <- lapply(bd_cluster_subsets, FUN = function(x){
  ggplot(x, aes(x = combined, y = Mean, group = geneid)) +
    geom_line(size = 0.1) +
    labs(x = "",
         y = "") +
    scale_x_discrete(labels = c("2-M", "2-I", "4-M", "4-I", "6-M", "6-I")) +
    scale_y_continuous(breaks = breaks_extended(3), limits = c(0, max(x$Mean))) +
...
W2691_name <- ggplot(W2691_cluster_subsets[[1]], aes(x = combined, y = Mean, group = geneid)) +
  geom_line(size = 0.1) +
  labs(x = "",
       y = "FPKM") +
  scale_x_discrete(labels = c("2-M", "2-I", "4-M", "4-I", "6-M", "6-I")) + 
  scale_y_continuous(breaks = breaks_extended(3), limits = c(0, max(W2691_cluster_subsets[[1]]$Mean))) +
  scale_color_manual(values = c("#636363")) +
  theme(panel.background = element_blank(), 
        axis.line = element_line(color = "black"), 
        axis.text.x = element_text(color = "black"),
...
for (j in 1:length(interest_list)) {
  ...
  W2691_name <- W2691_name + geom_line(data = subset(W2691_cluster_subsets[[1]], geneid == interest_list[j]), inherit.aes = TRUE, color = "#D0270C")
  
}


Sr9b_name <- ggplot(Sr9b_cluster_subsets[[1]], aes(x = combined, y = Mean, group = geneid)) +
  geom_line(size = 0.1) +
  labs(x = "",
       y = "") +
  scale_x_discrete(labels = c("2-M", "2-I", "4-M", "4-I", "6-M", "6-I")) + 
  scale_y_continuous(breaks = breaks_extended(3), limits = c(0, max(Sr9b_cluster_subsets[[1]]$Mean))) +
  scale_color_manual(values = c("#636363")) +
  theme(panel.background = element_blank(), 
        axis.line = element_line(color = "black"), 
        axis.text.x = element_text(color = "black"),
...
for (j in 1:length(interest_list)) {
  ...
  Sr9b_name <- Sr9b_name + geom_line(data = subset(Sr9b_cluster_subsets[[1]], geneid == interest_list[j]), inherit.aes = TRUE, color = "#D0270C")
  
}

Bd21_name <- ggplot(bd_cluster_subsets[[1]], aes(x = combined, y = Mean, group = geneid)) +
  geom_line(size = 0.1) +
  labs(x = "",
       y = "") +
  scale_x_discrete(labels = c("2-M", "2-I", "4-M", "4-I", "6-M", "6-I")) + 
  scale_y_continuous(breaks = breaks_extended(3), limits = c(0, max(bd_cluster_subsets[[1]]$Mean))) +
  scale_color_manual(values = c("#636363")) +
  theme(panel.background = element_blank(), 
        axis.line = element_line(color = "black"), 
        axis.text.x = element_text(color = "black"),
...
combined_plot <- egg::ggarrange(W2691_networks[[5]], Sr9b_networks[[2]], Bd21_networks[[7]], W2691_plotting[[3]], Sr9b_plotting[[3]], Bd21_plotting[[5]], W2691_networks[[1]], Sr9b_networks[[1]], single_point_plot, W2691_plotting[[1]], Sr9b_plotting[[1]], Bd21_plotting[[8]], W2691_networks[[1]], Sr9b_networks[[1]], Bd21_networks[[5]], W2691_name, Sr9b_name, Bd21_name, nrow = 6, ncol = 3, widths = c(1,1,1), heights = c(2,0.75,2,0.75,2,0.75))
combined_plot_w_text <- ggdraw(combined_plot) + draw_label("W2691", x = 0.2, y = 0.98, size = 10) +
  draw_label("W2691+Sr9b", x = 0.53, y = 0.98, size = 10) + 
  draw_label("Bd21-3", x = 0.85, y = 0.98, size = 10) +
  draw_label("n = 555", x = 0.14, y = 0.77, color = "black", size = 8) +
  draw_label("n = 2", x = 0.23, y = 0.77, color = "red", size = 8) +
  draw_label("n = 20", x = 0.47, y = 0.77, color = "black", size = 8) +
  draw_label("n = 1", x = 0.57, y = 0.77, color = "red", size = 8) +
  draw_label("n = 3", x = 0.80, y = 0.77, color = "black", size = 8) +
  draw_label("n = 1", x = 0.89, y = 0.77, color = "red", size = 8) +
  draw_label("n = 4521", x = 0.14, y = 0.44, color = "black", size = 8) +
  draw_label("n = 6", x = 0.23, y = 0.44, color = "red", size = 8) +
  draw_label("n = 3390", x = 0.47, y = 0.44, color = "black", size = 8) +
  draw_label("n = 10", x = 0.57, y = 0.44, color = "red", size = 8) +
  draw_label("n = 0", x = 0.80, y = 0.44, color = "black", size = 8) +
  draw_label("n = 1", x = 0.89, y = 0.44, color = "red", size = 8) +
  draw_label("n = 4521", x = 0.14, y = 0.11, color = "black", size = 8) +
  draw_label("n = 6", x = 0.23, y = 0.11, color = "red", size = 8) +
  draw_label("n = 3390", x = 0.47, y = 0.11, color = "black", size = 8) +
  draw_label("n = 10", x = 0.57, y = 0.11, color = "red", size = 8) +
  draw_label("n = 442", x = 0.80, y = 0.11, color = "black", size = 8) +
  draw_label("n = 1", x = 0.89, y = 0.11, color = "red", size = 8)

ggsave("Figure_6_together.tiff", combined_plot_w_text, device = "tiff", width = 7.5, height = 10, units = "in")
